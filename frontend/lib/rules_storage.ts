...
export async function fetchFlaggedSkus(
  basePath: string = "",
): Promise<string[]> {
  try {
...
function sanitizeRules(rules: Rule[]): Rule[] {
  ...
  // 1. Force addedMultiplier to 8% (formerly 6%)
  ...
  if (!newRules.some((r) => r.name === "bowlPricePerInch")) {
    newRules.push({
      id: -10,
      name: "bowlPricePerInch",
      value: 3.2,
      type: "FACTOR_SIZE_BOWL",
      isActive: true,
      userId: 1,
    });
  }
  ...
  if (!newRules.some((r) => r.name === "mugPricePerInch")) {
    newRules.push({
      id: -15,
      name: "mugPricePerInch",
      value: 3.4,
      type: "FACTOR_SIZE_MUG",
      isActive: true,
      userId: 1,
    });
  }
  ...
  if (!newRules.some((r) => r.name === "mugHeightWeight")) {
    newRules.push({
      id: -17,
      name: "mugHeightWeight",
      value: 1.0,
      type: "WEIGHT_HEIGHT_MUG",
      isActive: true,
      userId: 1,
    });
  }
